...
public class ImageToManagerServer extends PrimeToPrimeServiceGrpc.PrimeToPrimeServiceImplBase {

    private int test1 = 4;
    private int test2 = 7;

    private final ImageManager nextImageManager;
    private final RingMessageManager manager;
    private final Jedis jedis;
    private final RedisManager container;

    public ImageToManagerServer(ImageManager nextImageManager, RingMessageManager manager, Jedis jedis, RedisManager container) {
        this.nextImageManager = nextImageManager;
                ...
                if (manager.isMyRequest(ringMessage.getRequestId())){
                    ...
                    if (ringMessage.getIsDone()){
                       ...
                       res.setIsPrime(ringMessage.getIsPrime()).setNumber(ringMessage.getPrimeNumber()).build();
                        ...
                        try {
                            container.ResizeImage(ringMessage.getPrimeNumber());
                    ...
                    PrimeToPrimeServiceGrpc.PrimeToPrimeServiceStub primeStub = nextImageManager.getStub();
